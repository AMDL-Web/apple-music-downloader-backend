...
// HasConstraints reports whether any list-content filter is set (status, type,
// storefront, q, or time windows). Pagination and sort are ignored — they do
// not change which jobs belong in a filtered view.
func (f JobListFilter) HasConstraints() bool {
	return len(f.Statuses) > 0 ||
		len(f.Types) > 0 ||
		strings.TrimSpace(f.Storefront) != "" ||
		strings.TrimSpace(f.Query) != "" ||
		f.CreatedAfter != nil || f.CreatedBefore != nil ||
		f.UpdatedAfter != nil || f.UpdatedBefore != nil
}

// Matches reports whether job would appear in a ListJobs result for this
// filter (ignoring limit/offset/sort). Used by the overview SSE/WS feed to
// keep a filtered subscription in sync with GET /downloads.
func (f JobListFilter) Matches(job domain.Job) bool {
	if len(f.Statuses) > 0 {
		ok := false
	...
	if sf := strings.TrimSpace(f.Storefront); sf != "" && job.Storefront != sf {
		return false
	}
	if q := strings.TrimSpace(f.Query); q != "" {
		if !strings.Contains(strings.ToLower(job.Title), strings.ToLower(q)) &&
			!strings.Contains(strings.ToLower(job.Input), strings.ToLower(q)) {
			return false
		}
	}
	if f.CreatedAfter != nil && job.CreatedAt.Before(*f.CreatedAfter) {
		return false
	}
	if f.CreatedBefore != nil && job.CreatedAt.After(*f.CreatedBefore) {
		return false
	}
	if f.UpdatedAfter != nil && job.UpdatedAt.Before(*f.UpdatedAfter) {
		return false
	}
	if f.UpdatedBefore != nil && job.UpdatedAt.After(*f.UpdatedBefore) {
		return false
	}
	return true
}
