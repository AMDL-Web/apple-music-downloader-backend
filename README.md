...
- `POST /api/v1/quality`：不创建任务，探测单曲、专辑、歌单、艺人或电台 URL 内歌曲当前可用的编码与音质信息；它复用下载任务的区域校验、集合解析、元数据刷新、重试、并发调度和选流路径，并在加密媒体传输前停止。集合链接按 Apple Music 原顺序返回逐曲 `tracks`（含重复曲目），单曲继续返回兼容的 `song`/`qualities`。电台使用运行时 `catalog.media_user_token`。
